...
int Lane::attemptSpawn(int starting_site, Vehicle* vehicle_ptr, std::vector<Vehicle*>* vehicles) {
    ...
    if (attempted_position < (int) this->getSites().size() 
        ...
        // vehicle_ptr->setPosition(attempted_position);
        this->sites[vehicle_ptr->getPosition()].push_front(vehicle_ptr);
        vehicles->push_back(this->sites[vehicle_ptr->getPosition()].front());
    }
    // Return with no error
    return 0;
}
...
std::vector<std::deque<Vehicle*>> Lane::getSites(){
    return this->sites;
}